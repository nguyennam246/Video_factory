# CHỈ DẪN CHO Codex — DỰ ÁN CON `video/` (lập 23/07/2026)

**Điểm vào duy nhất: đọc `video/HANDOFF.md` TRƯỚC khi làm bất kỳ việc gì.**
Luật gốc repo (AGENTS.md gốc) vẫn áp dụng; file này chỉ thêm luật RIÊNG của video.
Cách chạy + định dạng file kịch bản: `video/README.md`.

**BOSS** = người dùng / chủ dự án. Mục đích: bộ video ngắn dạy **Codex**,
...
4. **Kịch bản viết theo khuôn 7 nhịp** (HANDOFF mục ⭐ CÔNG THỨC — phần trả giá đắt
   ...
   có số, đi chậm qua 1 lần chạy thử, câu chốt đối xứng. Trong **mỗi khóa**, chọn một
   ẩn dụ xuyên suốt phù hợp với chủ đề; khóa Claude Code cũ dùng "người thợ mới mỗi
   sáng", khóa Superpowers dùng "xây một căn nhà" (BOSS cho phép đổi 24/07). BOSS
   chốt: *"không cần làm quá ngắn — đúng là đủ"* (bài dạy 100-165 giây là bình thường).
...
## NGHI THỨC (hook trong `video/.Codex/` tự nhắc khi mở phiên tại video/)
...
- Hook khóa dữ liệu thô của repo gốc vẫn hiệu lực (gọi xuyên lên `../.Codex/hooks/`).
- Thước hook 0 API: `python3 video/.Codex/hooks/_thu_hook.py` — chạy sau khi sửa
...
| `video/kichban/` | Khóa Claude Code 10 bài + truyện + bài thử giọng |
| `video/kichban/superpowers/` | Khóa Superpowers cho Codex — 8 kịch bản + lộ trình |
...
| `../results/video/` | Video ra (gitignore — dựng lại được) |
