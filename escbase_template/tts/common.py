...
# ĐUÔI VIDEO — giây im lặng giữ lại sau khi dứt câu cuối.
#
# BOSS nghe PNJ_02 (25/07/2026): *"video hình như chưa hết mà đã dừng rồi"*. Đo ra đúng:
# tổng lời 97,176s, video 97,200s ⇒ đuôi 0,024s. Một giây cuối vẫn còn tiếng ở max
# −4,8 dB, tức giọng đọc chạm đúng khung hình cuối rồi cắt phựt. Không phải cảm giác.
# Bệnh có ở MỌI bài trước, chỉ là chưa ai gọi tên.
TAIL_SECONDS = 1.4
...
async def generate_project_tts(
    ...
    *,
    ...
    for i, line in enumerate(lines):
        # `line` = chữ GỐC (đi vào timing.json -> phụ đề).
        # `doc`  = chữ ĐỌC (gửi engine). Cache phải khoá theo `doc`, KHÔNG theo `line`:
        # khoá theo `line` thì sửa tts/phat_am.json xong tiếng cũ vẫn được dùng lại,
        # lỗi im lặng đúng kiểu đã trả giá ở khâu phụ đề 24/07.
        doc = doc_am(line)
        ...
        text_cache_matches = text_cache_file.exists() and text_cache_file.read_text(encoding="utf-8") == doc
        expected_meta = cache_metadata(i, doc) if cache_metadata else None
        ...
        if not force and meta_cache_matches and audio_file.exists() and audio_file.stat().st_size > 0:
            ...
            # Gửi chữ ĐỌC cho engine, nhưng bên dưới timing.json vẫn ghi `line` GỐC
            # ⇒ phụ đề hiện "PNJ", máy đọc "Pi En Giây". Xem tts/phat_am.py.
            await maybe_await(line_generator(i, doc, generation_file, srt_file))
            if use_post_speed:
            ...
            text_cache_file.write_text(doc, encoding="utf-8")
    ...
    # ĐUÔI: giữ slide cuối thêm một nhịp sau khi dứt câu. Dùng lại đúng cơ chế
    # padding_by_slide có sẵn (write_silence_file + concat_line_audio) nên timing.json,
    # file tiếng ghép và video vẫn khớp nhau — đừng vá bằng cách nối im lặng vào mp4
    # sau khi render, làm vậy là lệch timing.json.
    if tail_seconds > 0 and line_durations:
        cuoi = len(lines) - 1
        tong_pad = padding_by_slide.get(cuoi, 0.0) + tail_seconds
        padding_by_slide[cuoi] = tong_pad
        write_silence_file(output_dir, f"silence_{cuoi}.mp3", tong_pad)
        line_durations[cuoi]["duration"] += tail_seconds
        print(f"  ↳ ĐUÔI: giữ slide {cuoi + 1} thêm {tail_seconds:.2f}s sau câu cuối "
              f"(tổng im lặng slide này {tong_pad:.2f}s)")
