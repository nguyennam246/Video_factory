...
from tts.common import TAIL_SECONDS   # một nguồn sự thật cho độ dài đuôi, đừng chép số
from tts.phat_am import doc_am        # chỉ áp lên chữ ĐỌC, không áp lên chữ phụ đề
...
async def main() -> None:
    ...
    # HAI BẢN CỦA CÙNG MỘT LỜI — đừng gộp lại:
    #   lines      = chữ GỐC  -> đi vào timing.json -> ra PHỤ ĐỀ ("PNJ")
    #   lines_doc  = chữ ĐỌC  -> dùng để căn với tiếng máy đã đọc ("Pi En Giây")
    # Căn bằng bản gốc là sai chỗ cắt: Whisper nghe được "Pi En Giây" chứ không phải "PNJ".
    lines = [l.strip() for l in script_path.read_text(encoding="utf-8").splitlines() if l.strip()]
    lines_doc = [doc_am(l) for l in lines]
    ...
    split_points = align_words_to_lines(words, lines_doc)
    ...
    # ĐUÔI — giữ slide cuối thêm một nhịp sau khi dứt câu.
    # Nhánh edge "full rồi cắt" đi qua ĐÂY, không qua tts/common.py, nên phải vá cả hai
    # chỗ. Vá thiếu chỗ này thì đuôi chỉ có ở nhánh per-slide và ElevenLabs — im lặng.
    padding_by_slide: dict[int, float] = {}
    if TAIL_SECONDS > 0 and line_durations:
        cuoi = num_slides - 1
        padding_by_slide[cuoi] = TAIL_SECONDS
        write_silence_file(output_dir, f"silence_{cuoi}.mp3", TAIL_SECONDS)
    ...
    concat_file = concat_segments(output_dir, num_slides, padding_by_slide)
